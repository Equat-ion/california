...
function Icon({
  ...
  const materialCommunityIconName = SF_SYMBOLS_TO_MATERIAL_COMMUNITY_ICONS[name as keyof typeof SF_SYMBOLS_TO_MATERIAL_COMMUNITY_ICONS];
  if (materialCommunityIconName) {
    return <MaterialCommunityIcons name={materialCommunityIconName} size={size} color={defaultColor} {...props} />;
  }

  const materialIconName = SF_SYMBOLS_TO_MATERIAL_ICONS[name as keyof typeof SF_SYMBOLS_TO_MATERIAL_ICONS];
  if (materialIconName) {
